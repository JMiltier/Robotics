...
void setup() {
  ...
  world_map[1][0] = 0; // Example of setup code to indicate an obstacle at grid position (0,1)
...
/**************************************
 *Coordinate Transform Functions (END)*
 **************************************/
...
// vertex_dest is the next possible location to move
byte get_travel_cost(int vertex_source, int vertex_dest) {
  ...
  if ((world_map[d_i][d_j])==0) return BIG_NUMBER;
...
//  sparki.println(dTheta);
  ...
  sparki.print("current state: "); sparki.println(current_state);
  sparki.print("path value: "); sparki.print(path[u]);
  sparki.updateLCD();
}
...
void loop () {
  ...
  // STATE_START 1
  // STATE_HAS_PATH 2
  // STATE_SEEKING_POSE 3
  ****************************/

  int di, dj;

  /********************************
  previous goal_change if parameter
  current_goal defined globally to [ ij->vidx(IGS_i, IGS,j) ]
  current_goal != ij_coordinates_to_vertex_index(goal_i, goal_j)
  ********************************/
  
  //STATE_START: Compute current vertex to goal position while goal_changed == TRUE
  //xy_coordinates_to_ij_coordinates(pose_x, pose_y, &source_i, &source_j) && goal_changed == TRUE
  if(current_state == STATE_START || goal_changed == TRUE){
    prev = run_dijkstra(world_map, ij_coordinates_to_vertex_index(source_i, source_j));
    path = reconstruct_path(prev, ij_coordinates_to_vertex_index(source_i, source_j),
                            ij_coordinates_to_vertex_index(goal_i, goal_j));

    current_state = STATE_HAS_PATH;
   
    goal_changed = FALSE;       //update goal change status
    delete prev; prev = NULL;   //free memory associated with prev array
  }else if(current_state == STATE_HAS_PATH){        //assigns a goal pose for robot to move towards
    //TODO
    vertex_index_to_ij_coordinates(path[u++], &di, &dj);
    ij_coordinates_to_xy_coordinates(di, dj, &dest_pose_x, &dest_pose_y);
    current_state = STATE_SEEKING_POSE; // move to next location
  }else if(current_state == STATE_SEEKING_POSE){    //implementation of inverse kinematics
    //TODO
    compute_IK_errors();
    compute_IK_wheel_rotations();
    set_IK_motor_rotations();
    if(path[u+1] == -1) current_state = STATE_PATH_END; // at goal position
    if (is_robot_at_IK_destination_pose()) {
      moveStop();
      current_state = STATE_HAS_PATH;
    }
  }else{        //Sparki at goal position, STATE_PATH_END (or failure)
    moveStop();
